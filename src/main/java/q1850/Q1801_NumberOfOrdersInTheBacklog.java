 ...
 * price, which is 15 and these 2 sell orders are removed from the backlog. The 3rd order is matched with the sell order
 * of the least price, which is 25 and this sell order is removed from the backlog. Then, there are no more sell orders
 * in the backlog, so the 4th order is added to the backlog.
 * Finally, the backlog has 5 buy orders with price 10, and 1 buy order with price 30. So the total number of orders in
 * the backlog is 6.
 *
 * Example 2:
 * (图Q1801_PIC2.png)
 ...
 * Output: 999999984
 * Explanation: Here is what happens with the orders:
 * - 10^9 orders of type sell with price 7 are placed. There are no buy orders, so the 10^9 orders are added to the
 * backlog.
 * - 3 orders of type buy with price 15 are placed. They are matched with the 3 sell orders with the least price which
 * is 7, and those 3 sell orders are removed from the backlog.
 ...
 * that buy order is removed from the backlog.
 * Finally, the backlog has (1000000000-3) sell orders with price 7, and (999999995-1) buy orders with price 5. So the
 * total number of orders = 1999999991, which is equal to 999999984 % (10^9 + 7).
 *
 * Constraints:
 *
...
public class Q1801_NumberOfOrdersInTheBacklog {
    ...
    public int getNumberOfBacklogOrders(int[][] orders) {
        ...
        for (int[] order : orders) {
            if (order[2] == 0) {
                // 买
                while (order[1] > 0 && !sellOrders.isEmpty()) {
                    int[] sell = sellOrders.peek();
                    if (sell[0] <= order[0]) {
                        if (sell[1] <= order[1]) {
                            sellOrders.poll();
                            order[1] -= sell[1];
                ...
                // 卖
                while (order[1] > 0 && !buyOrders.isEmpty()) {
                    int[] buy = buyOrders.peek();
                    if (buy[0] >= order[0]) {
                        if (buy[1] <= order[1]) {
                            buyOrders.poll();
                            order[1] -= buy[1];
