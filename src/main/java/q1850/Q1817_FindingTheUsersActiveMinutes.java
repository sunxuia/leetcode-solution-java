 ...
 * Output: [0,2,0,0,0]
 * Explanation:
 * The user with ID=0 performed actions at minutes 5, 2, and 5 again. Hence, they have a UAM of 2 (minute 5 is only
 * counted once).
...
public class Q1817_FindingTheUsersActiveMinutes {
    ...
    public int[] findingUsersActiveMinutes(int[][] logs, int k) {
        ...
        for (int[] log : logs) {
            ...
            map.computeIfAbsent(user, u -> new HashSet<>()).add(time);
        }
        int[] res = new int[k];
    ...
    public DataExpectation example1 = DataExpectation
            .createWith(new int[][]{{0, 5}, {1, 2}, {0, 2}, {0, 5}, {1, 3}}, 5)
            .expect(new int[]{0, 2, 0, 0, 0});

    @TestData
    public DataExpectation example2 = DataExpectation
            .createWith(new int[][]{{1, 1}, {2, 2}, {2, 3}}, 4)
