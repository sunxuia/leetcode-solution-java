 ...
 * the ties are resolved. If two or more teams are still tied after considering all positions, we rank them
 * alphabetically based on their team letter.
 *
 * Given an array of strings votes which is the votes of all voters in the ranking systems. Sort all teams according to
...
public class Q1366_RankTeamsByVotes {
    ...
    public String rankTeams(String[] votes) {
        ...
        for (int i = 0; i < n; i++) {
            sort[i] = votes[0].charAt(i) - 'A';
        }
        Arrays.sort(sort, (a, b) -> {
            for (int i = 0; i < n; i++) {
                if (ranks[a][i] != ranks[b][i]) {
                    return ranks[b][i] - ranks[a][i];
                }
            }
            return a - b;
        });
    ...
    public DataExpectation example1 = DataExpectation
            .create(new String[]{"ABC", "ACB", "ABC", "ACB", "ACB"})
            .expect("ACB");

    @TestData
    public DataExpectation example2 = DataExpectation
            .create(new String[]{"WXYZ", "XYZW"})
            .expect("XWYZ");

    @TestData
    public DataExpectation example3 = DataExpectation
            .create(new String[]{"ZMNAGUEDSJYLBOPHRQICWFXTVK"})
            .expect("ZMNAGUEDSJYLBOPHRQICWFXTVK");

    @TestData
    public DataExpectation example4 = DataExpectation
            .create(new String[]{"BCA", "CAB", "CBA", "ABC", "ACB", "BAC"})
            .expect("ABC");

    @TestData
    public DataExpectation example5 = DataExpectation
            .create(new String[]{"M", "M", "M", "M"})
            .expect("M");
