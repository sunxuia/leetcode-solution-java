...
public class Q1629_SlowestKey {
    ...
    public char slowestKey(int[] releaseTimes, String keysPressed) {
        ...
        char res = 'a';
        int maxTime = 0;
        for (int i = 0, p = 0; i < n; p = releaseTimes[i++]) {
            int time = releaseTimes[i] - p;
            if (time > maxTime || time == maxTime && res < keysPressed.charAt(i)) {
                maxTime = time;
    ...
    public DataExpectation example1 = DataExpectation
            .createWith(new int[]{9, 29, 49, 50}, "cbcd")
            .expect('c');

    @TestData
    public DataExpectation example2 = DataExpectation
            .createWith(new int[]{12, 23, 36, 46, 62}, "spuda")
            .expect('a');
