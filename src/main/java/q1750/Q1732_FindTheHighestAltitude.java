...
public class Q1732_FindTheHighestAltitude {
    ...
    public int largestAltitude(int[] gain) {
        int res = 0, curr = 0;
        for (int offset : gain) {
            res = Math.max(res, curr += offset);
        }
        return res;
    }

    @TestData
    public DataExpectation example1 = DataExpectation.create(new int[]{-5, 1, 5, 0, -7}).expect(1);

    @TestData
    public DataExpectation example2 = DataExpectation.create(new int[]{-4, -3, -2, -1, 4, 3, 2}).expect(0);
