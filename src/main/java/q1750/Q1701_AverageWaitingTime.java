 ...
 * 2) The second customer arrives at time 2, the chef takes his order and starts preparing it at time 3, and finishes at
 * time 8, so the waiting time of the second customer is 8 - 2 = 6.
...
public class Q1701_AverageWaitingTime {

    @Answer
    public double averageWaitingTime(int[][] customers) {
        double timeSum = 0, finishTime = 0;
        for (int[] customer : customers) {
            int arrival = customer[0];
            int time = customer[1];
            finishTime = Math.max(finishTime, arrival) + time;
            timeSum += finishTime - arrival;
        }
    ...
    public DataExpectation example1 = DataExpectation
            .create(new int[][]{{1, 2}, {2, 5}, {4, 3}})
            .expectDouble(5.00000);

    @TestData
    public DataExpectation example2 = DataExpectation
            .create(new int[][]{{5, 2}, {5, 4}, {10, 3}, {20, 1}})
            .expectDouble(3.25000);
