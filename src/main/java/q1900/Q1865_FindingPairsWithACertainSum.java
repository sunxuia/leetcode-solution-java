 ...
 */
public class Q1865_FindingPairsWithACertainSum {

    private static class FindSumPairs {

        int[] nums1, nums2;

        // 题目没规定 nums2[i] 永远 ≤ 10_0000
        Map<Integer, Integer> bucket2 = new HashMap<>();
        ...
        public FindSumPairs(int[] nums1, int[] nums2) {
            ...
            this.nums2 = nums2;
            for (int num : nums2) {
                bucket2.merge(num, 1, Integer::sum);
            }
        }

        public void add(int index, int val) {
            bucket2.compute(nums2[index], (k, v) -> v - 1);
            nums2[index] += val;
            bucket2.merge(nums2[index], 1, Integer::sum);
        }

        public int count(int tot) {
            int res = 0;
            for (int i = 0; i < nums1.length && nums1[i] < tot; i++) {
                res += bucket2.getOrDefault(tot - nums1[i], 0);
            }
            return res;
        }
    }
    ...
    public void testMethod() {
        ...
        res = findSumPairs.count(7);
        AssertUtils.assertEquals(8, res);

        findSumPairs.add(3, 2);
        res = findSumPairs.count(8);
        AssertUtils.assertEquals(2, res);

        res = findSumPairs.count(4);
        AssertUtils.assertEquals(1, res);

        findSumPairs.add(0, 1);
        findSumPairs.add(1, 1);
        res = findSumPairs.count(7);
        AssertUtils.assertEquals(11, res);
    }
}
